...
class Imager():
    ...
    def __init__(self, pid=None, image=False, width=100, height=100, background='black',
                 mode='RGB'):
        self.pid = pid
    ...
    def load_image(self):
        self.image = Image.open(self.pid)  # the image is actually loaded as needed (automatically by PIL)
    ...
    def dump_image(self, pid, type='gif'):
        pid = pid.split('.')
        type = pid[1] if len(pid) < 1 else type
        self.image.save(pid[0]+"."+type, format=type)
...
class ArtShow:
    ...
    def __init__(self, pid1, pid2, pid3, size):
        self.im1 = Imager(pid1).resize(size, size)
        self.im2 = Imager(pid2).resize(size, size)
        self.im3 = Imager(pid3).resize(size, size)
        self.size = size

    def SomethingFromImage(self, im1, im2):
        im1 = im1.get_image().rotate(90)
        im3 = Image.blend(im1, im2.get_image(), 0.5)
        return im3

    def SomethingFromImageFilter(self,im, blur):
        im1 = im.get_image().filter(ImageFilter.GaussianBlur(radius=blur))
        return im1

    def SomethingFromImageOps(self, im):
        im1 = ImageOps.expand(im.get_image(), 50)
        return im1
    ...
    def MasterPiece(self):
        ...
        im2 = Imager()
        im3 = Imager()
        im1.set_image(self.SomethingFromImage(self.im1,self.im2))
        im2.set_image(self.SomethingFromImage(im1,self.im3))
        im3.set_image(self.SomethingFromImageFilter(im2,5))
        im3.set_image(self.SomethingFromImageOps(im3))
        im3.display()


def main():
    pid1 = 'images/kdfinger.gif'
    pid2 = 'images/fisheggs.jpeg'
    pid3 = 'images/minions.gif'
    artshow = ArtShow(pid1,pid2,pid3,1000)
    artshow.MasterPiece()
main()
